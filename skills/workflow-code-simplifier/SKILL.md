...
description: Use when the task is to simplify code, clean up recently changed code, refactor for clarity, or improve readability without changing behavior, especially as a final cleanup pass after the main work is already green
---
...
Use it for non-behavioral cleanup after the task boundary is already understood. By default, apply it once near the end of implementation, after the main behavior is already green. It complements `workflow-test-driven-development` and `workflow-verification-before-completion`; it does not replace them.
...
- the main implementation is already green and now needs a final scoped cleanup pass

Use it earlier only when:
- local cleanup is needed to keep the next implementation step understandable
- the cleanup stays inside the current boundary and does not interrupt the main task flow
